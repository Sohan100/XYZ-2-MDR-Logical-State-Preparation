...
MDRCircuit_cell20.py
----------------------------------------------------------------------------
Legacy documentation scratch snapshot for MDRCircuit cell20.
"""
...
class MDRCircuit:
    ...
    Args:
    stabilizers (List[str] | None): List of stabilizer generator strings. If
    None, a default set is used. toggles (List[str] | None): List of recovery
    toggle strings. If None, a default set is used. ancillas (int): Number of
    ancilla qubits to use (min 1). p_spam (float): Probability of SPAM noise.
    p_x (float): Probability of X error during idling. p_y (float): Probability
    of Y error during idling. p_z (float): Probability of Z error during
    idling. g1_x (float): Prob. of X error after each 1-qubit gate. g1_y
    (float): Prob. of Y error after each 1-qubit gate. g1_z (float): Prob. of Z
    error after each 1-qubit gate. gate_noise_2q (List[float] | None): 15
    two-qubit error probabilities. psi_circuit (stim.Circuit | None): initial
    ...
    def _insert_pauli_channel_2(
        self, circ: stim.Circuit, q_a: int, q_d: int, noise: List[float]
    ):
        """
    ...
    def _gate(self, circ: stim.Circuit, name: str, tgts: List[int]):
        """
        ...
        if self.g1_x == self.g1_y == self.g1_z == 0 and not any(
            self.gate_noise_2q
        ):
            return
    ...
    def _spam_gate(self, circ: stim.Circuit, name: str, tgts: List[int]):
        ...
        if name == "R":
            circ.append_operation(name, tgts)
            self._insert_pauli_channel(circ, tgts, self.p_spam, 0, 0)
