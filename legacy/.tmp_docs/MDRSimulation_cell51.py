...
class MDRSimulation:
    ...
    This class precomputes, for each specified stabilizer and logical operator,
    the distribution of absolute mean-estimates obtained by repeating an N-shot
    experiment (`shots_per_measurement`) `num_replicates` times per MDR round
    (0…`total_mdr_rounds`), and caches both the full replicate lists (for
    violin plots) and summary statistics (mean ± sample-stddev). It also
    computes an overall average expectation per operator (mean of the per-round
    means).
    ...
    Attributes: prepare_circuit_function (Callable[[], stim.Circuit]): Factory
    returning a fresh stim.Circuit to prepare the initial state. mdr_circuit
    (stim.Circuit): stim.Circuit template implementing one round of MDR.
    stabilizer_pauli_strings (List[str]): Sparse Pauli-string specifications
    for stabilizer measurements. logical_pauli_strings (Dict[str, str]):
    Mapping from logical operator labels to sparse Pauli specs.
    shots_per_measurement (int): Number of Monte-Carlo samples per expectation
    estimate. total_mdr_rounds (int): Maximum number of MDR rounds to simulate
    (inclusive of round 0). num_replicates (int): Number of independent N-shot
    repeats per round for violin distributions. last_results_storage
    (Dict[str,Dict[str,Tuple[List[int],List[float]]]]): Backward-compatible
    single-run mean-value curves. _replicate_means_stabilizers
    (Dict[str,Dict[int,List[float]]]): Raw lists of absolute replicate-mean
    estimates per stabilizer & round. _replicate_means_logicals
    (Dict[str,Dict[int,List[float]]]): Raw lists of absolute replicate-mean
    estimates per logical & round. _stats_stabilizers
    (Dict[str,Dict[str,List[float]]]): Summary statistics for each stabilizer:
    `'rounds'`, `'centers'` (means), and `'stds'` (sample-stddevs).
    _stats_logicals (Dict[str,Dict[str,List[float]]]): Summary statistics for
    each logical operator. _avg_stabilizers (Dict[str, float]): Overall average
    expectation per stabilizer (mean of `'centers'`). _avg_logicals (Dict[str,
    float]): Overall average expectation per logical operator.

    Methods: spec_to_measurement_ops(pauli_spec: str) -> List[Tuple[str,int]]:
    Convert a sparse Pauli string (e.g. "X0 Z1") into a list of stim
    measurement operations [(gate, qubit_index), …].

    compute_parity_expectation(circuit, measurement_ops) -> float: Append the
    given measurement operations to `circuit`, sample `shots_per_measurement`
    bit-strings, compute the parity product of the measured bits, map to ±1
    eigenvalues, and return the absolute mean |⟨O⟩|.

    calculate_replicated_means_vs_rounds(pauli_spec: str) -> Dict[int,
    List[float]]: For each MDR round r, build the circuit with r rounds, then
    run `num_replicates` independent N-shot experiments. Return a mapping {r →
    [|mean₁|, |mean₂|, …]} of all replicate absolute-mean values.

    calculate_expectation_vs_rounds(pauli_spec: str) -> (List[int],
    List[float]): For each MDR round r, build the circuit and perform a single
    N-shot experiment. Return two lists: the round indices and the
    corresponding |⟨O⟩| estimates.

    calculate_many_expectations(spec_map: Mapping[str,str]) -> Dict[str,
    (List[int],List[float])]: Apply `calculate_expectation_vs_rounds` to each
    Pauli spec in `spec_map`, returning a dict label → (rounds, means).
    ...
    plot_data(data_map, title, x_label, y_label, **kwargs) -> None: Generic
    renderer for precomputed violin distributions: • Plots only markers (no
    connecting lines) for the summary means. • Overlays violins of the
    replicate-mean distributions, with mean & extrema lines, all colored to
    match each marker. • Optionally scatters every replicate point inside its
    violin. • Handles saving to disk if `save_path` is provided.

    plot_expectation(pauli_specification=None, save_path=None, **kw) -> None:
    Single-operator plotting: if no spec is given, plots all stabilizers; if a
    string is given, plots that one; if a list is given, plots each. Delegates
    to `plot_data` (optionally adding violins) but uses fresh single-run
    sampling, not the precomputed cache.

    plot_many_expectations(pauli_specs, save_path=None, **kw) -> None:
    Multi-operator plotting: takes either a list of Pauli strings or a mapping
    label→spec, performs fresh single-run sampling for each, then delegates to
    `plot_data`.

    plot_group(save_path_stabilizers=None, save_path_logicals=None, **kw) ->
    None: Render two figures—one for all stabilizers, one for all logicals—
    using the precomputed violin distributions and summary means. Saves each
    figure if a corresponding `save_path_*` is provided.

    results() -> None: Print tabular, precomputed statistics for each operator:
    Round | Mean | StdDev followed by the overall average expectation for that
    operator.
    """
    ...
    def __init__(
        ...
        Args:
        mdr (MDRCircuit): MDRCircuit instance containing the circuit
        preparation stabilizer_pauli_strings (List[str]): List of sparse
        stabilizer Pauli specifications. logical_pauli_strings (Dict[str,
        str]): Dictionary mapping logical labels to Pauli specifications.
        shots_per_measurement (int, optional): Number of shots per expectation
        estimate. Default is 1000. total_mdr_rounds (int, optional): Number of
        MDR rounds (inclusive of 0). Default is 10. num_replicates (int,
        optional): Number of repeats per round for violin distributions.
        Default 30.
    ...
    def compute_parity_expectation(
        self, circuit: stim.Circuit, measurement_ops: List[Tuple[str, int]]
    ) -> float:
        """
        ...
        samples = sampler.sample(
            shots=self.shots_per_measurement, bit_packed=False
        )
    ...
    def calculate_replicated_means_vs_rounds(
        ...
        for r in range(self.total_mdr_rounds + 1):
            ...
            for _ in range(self.num_replicates):
                ...
                circ = base.copy()
                absolute_eigen = self.compute_parity_expectation(
                    circ, measurement_ops
                )
    ...
    def calculate_expectation_vs_rounds(
        ...
        Returns:
        Tuple[List[int], List[float]]: (List of rounds, List of mean values per
        round)
        """
        # Convert Pauli string to measurement operations
    ...
    def calculate_group_data(
        ...
        Returns:
        Dict[str, Dict[str, Tuple[List[int], List[float]]]]: Dictionary with
        keys 'stabilizers' and 'logicals', each mapping to a dictionary of
        label → (rounds, means).
        """
    ...
    def plot_data(
        ...
        **legend_kwargs,
        ...
        spread of replicate means, with mean and extrema lines colored to match
        the corresponding marker. Individual replicate points can also be
        scattered within each violin for additional detail. The function
        supports saving the resulting figure to disk and customizing legend
        placement and violin plot appearance.

        Args:
        data_map (Dict[str, Tuple[List[int], List[float]]]): Mapping from
        operator label to (round indices, mean values). title (str): Title for
        the plot. x_label (str): Label for the x-axis. y_label (str): Label for
        the y-axis. legend_location (str, optional): Location of the legend.
        Default is 'upper left'. bbox_to_anchor (Tuple[float, float],
        optional): Anchor point for the legend. Default is (1.02, 1). save_path
        (str, optional): If provided, saves the figure to this path.
        show_violin (bool, optional): Whether to overlay violin plots for
        replicate distributions. Default is True. violin_kwargs (Dict,
        optional): Additional keyword arguments for the violin plot.
        **legend_kwargs: Additional keyword arguments for the legend.
        ...
        for idx, label in enumerate(data_map):
            ...
            if (
                hasattr(self, "_stats_stabilizers")
                and label in self._stats_stabilizers
            ):
                ...
                hasattr(self, "_stats_logicals")
                and label in self._stats_logicals
            ):
            ...
            if show_violin and dists is not None:
                ...
                for r, vals in zip(rounds, dists):
                    ax.scatter(
                        [r] * len(vals), vals, color=colour, alpha=0.05, s=4
                    )
    ...
    def plot_expectation(
        ...
        **plot_kwargs,
        ...
        Args:
        pauli_specification (Union[str, List[str]], optional): Pauli
        specification(s) to plot. If None, uses all stabilizers. save_path
        (str, optional): If provided, saves the figure to this path.
    ...
    def plot_many_expectations(
        ...
        **plot_kwargs,
        ...
        Bespoke multi-operator plotting: computes fresh single-run curves, then
        delegates to plot_data.

        Args:
        pauli_specifications (Union[List[str], Mapping[str, str]]): List or
        mapping of Pauli specifications to plot. save_path (str, optional): If
        provided, saves the figure to this path. **plot_kwargs: Additional
        keyword arguments for plot_data.
    ...
    def plot_group(
        ...
        **plot_kwargs: Additional keyword arguments for plot_data.
        ...
        self.plot_data(
            self.last_results_storage["stabilizers"],
        ...
        self.plot_data(
            self.last_results_storage["logicals"],
    ...
    def plot_multi_fidelity(
        ...
        *,
        ...
        Plots only isolated markers with ±1σ error‐bars, plus optional violin +
        replicate overlays.  Supports 'stabilizer', 'logical', or 'both'.

        Args:
        sims:            Mapping label → MDRSimulation (one column per).
        category:        'stabilizer', 'logical', or 'both'. operators:
        None (all), a single str, or a list of str. show_violin:     If True,
        overlay violin plots. show_replicates: If True, scatter individual
        replicate points. figsize:         Figure size (width, height) in
        inches. save_path:       If provided, save figure (dpi=300).

        Raises:
        ValueError: If `sims` is empty, or `category` invalid, or any
        `operators` not found.
        """
        ...
        fig, axes = plt.subplots(
            nrows, ncols, figsize=figsize, squeeze=False, sharex="col"
        )
        ...
        for row, cat in enumerate(cats):
            ...
            dist_attr = (
                "_replicate_means_stabilizers"
                if cat == "stabilizer"
                else "_replicate_means_logicals"
            )
    ...
    def results(self) -> None:
        """
        Print precomputed tabular statistics (round | mean | stddev) and
        overall average expectation for each operator.
        ...
        for category, (stats_map, avg_map) in (
            ...
            for label, stats in stats_map.items():
                ...
                print("-----+---------+--------")
                for r, m, s in zip(
                    stats["rounds"], stats["centers"], stats["stds"]
                ):
    ...
    def tau_table(
        ...
        *,
        ...
        """
        Compute and display per-round across-operator dispersion τ_r and mean
        μ_r.

        Definitions (for the chosen category): For each round r let the set of
        replicate-mean fidelities across operators be { m_{k,r} }_{k=1}^K,
        where m_{k,r} is the plotted mean for operator k at round r (already
        stored in stats['centers']).

        μ_r  = (1/K) Σ_k m_{k,r} τ_r  = sqrt( Σ_k (m_{k,r} - μ_r)^2 / (K-1) )
        (sample std across operators)

        We report: * Per-round μ_r and τ_r. * Average τ across rounds
        (excluding r=0 if requested). * Average μ across rounds (excluding r=0
        if requested). * Both also including round 0 for reference.

        Args:
        category (str): 'stabilizer' or 'logical'. ignore_round0 (bool,
        optional): Exclude round 0 from the averages (default True).
        return_dict (bool, optional): If True, return a dict of computed
        quantities. precision (int, optional): Number of decimal places in
        printed table. label (str, optional): Optional label (e.g. noise model
        name) for header.

        Returns:
        Optional[Dict[str, Any]]: { 'rounds': [...], 'tau': [...], 'mu': [...],
        'avg_tau_excluding_round0': float, 'avg_tau_including_round0': float,
        'avg_mu_excluding_round0': float, 'avg_mu_including_round0': float } if
        return_dict=True, else None.
        ...
        stats_map = (
            self._stats_stabilizers
            if category == "stabilizer"
            else self._stats_logicals
        )
        ...
        for idx_r, r in enumerate(rounds):
            vals_r = [
                op_stats["centers"][idx_r] for op_stats in stats_map.values()
            ]
    ...
    def compare_tau(
        ...
        *,
        ...
        """
        Compare average across-operator dispersion τ and average mean μ among
        multiple simulations.

        For each simulation and the chosen category: * Compute τ_r and μ_r per
        round. * Report averages excluding (optionally) round 0 and including
        it.

        Args:
        sims (Dict[str, MDRSimulation]): Mapping simulation label → instance.
        category (str): 'stabilizer' or 'logical'. ignore_round0 (bool,
        optional): Exclude round 0 from averages (default True). precision
        (int, optional): Printing precision. return_table (bool, optional): If
        True, return list of row dicts.

        Returns:
        Optional[List[Dict[str, float]]]: Each row has: simulation, avg_tau,
        avg_tau_including_round0, avg_mu, avg_mu_including_round0 (if
        return_table=True).
        ...
        for label, sim in sims.items():
            stats_map = (
                sim._stats_stabilizers
                if category == "stabilizer"
                else sim._stats_logicals
            )
            first_stats = next(iter(stats_map.values()))
            rounds = first_stats["rounds"]
            ...
            for idx_r, r in enumerate(rounds):
                arr = np.array(
                    [
                        op_stats["centers"][idx_r]
                        for op_stats in stats_map.values()
                    ],
                    dtype=float,
                )
        ...
        print(
            f"{'Simulation':<30} | {'avg ?':>10} | "
            f"{'avg ? (incl r0)':>16} | {'avg ?':>10} | "
            f"{'avg ? (incl r0)':>16}"
        )
        print("-" * 94)
        for row in rows:
