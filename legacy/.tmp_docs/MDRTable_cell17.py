...
class MDRTable:
    ...
    def __init__(
        self, distance: int, save_filename: Optional[str] = None
    ) -> None:
        """
        Initialize the MDRTable, compile operator data, and handle file I/O.

        Args:
        distance: Code distance (odd integer >= 3). save_filename: Optional CSV
        output path.
    ...
    def _compile_data(self) -> None:
        ...
        # 3. Generate Toggles
        # FIX: Using positional arguments to match the RobustToggleGenerator
        # definition
    ...
    def _handle_save_request(self) -> None:
        ...
        This mirrors the notebook workflow that avoids overwriting precomputed
        table files.
        ...
        if self.save_filename and os.path.exists(self.save_filename):
            print(
                ...
                f"{os.path.abspath(self.save_filename)}"
            )
    ...
    def get_toggles(self) -> Tuple[List[str], str]:
        ...
        The first element corresponds to stabilizers only; the second is the
        dedicated Logical-X toggle.
    ...
    def save_latex(self, filename: str) -> None:
        ...
        with open(filename, "w") as f:
            f.write(latex_code)
        print(
            f"   -> LaTeX table saved locally to: {os.path.abspath(filename)}"
        )
