...
class MdrNoiseSweep:
    ...
    Attributes
    ----------
    two_qubit_index : Dict[str, int] Maps two-qubit noise keys (e.g. 'ZZ') to
    ...
    (e.g. 'g1_z', 'p_x'). sync : bool True if using a single shared list of
    values for all parameters (simultaneous sweep). param_names : list[str]
    Names of noise parameters being varied in this sweep. param_values_list :
    list[float] Shared list of noise values when sync=True. param_values_map :
    Dict[str, list[float]] Per-parameter lists of noise values when sync=False
    (asynchronous sweep). param_combos : list[tuple[float,...]]
    Cartesian-product of parameter values to sweep. results : Dict[tuple,
    Dict[int, Dict[str, float]]] Mean ⟨|O|⟩ values for each combo, round, and
    operator. Structure: results[param_combo][round_index][operator_label] =
    mean_val results_std : Dict[tuple, Dict[int, Dict[str, float]]] Sample
    standard deviations of ⟨|O|⟩ for each combo, round, and operator.

    Methods
    -------
    __init__(...) Configure sweep settings, build parameter combinations, and
    run (or load). _perform_sweep() Internal: run MDRSimulation per combo,
    applying 1q/2q splitting logic. save_results(filename) Export flattened
    simulation results to a CSV file. load_results(filename) Import results
    from a CSV file and reconstruct class state for plotting.
    plot_expectations_vs_param(...) Plot mean ⟨|O|⟩ vs noise parameter p.
    plot_error_rates_vs_param(...) Plot error rate = 1 - ⟨|O|⟩ vs noise p.
    plot_multi(...) Static: overlay or grid of multiple sweeps of ⟨|O|⟩ vs p.
    plot_error_multi(...) Static: overlay or grid of multiple sweeps of error
    rates with bars. table_round_delta(...) Print table comparing fidelity
    ...
    def __init__(
        ...
        Can operate in two modes: 1. **Simulation Mode**: Runs the sweep and
        optionally saves data. 2. **Load Mode**: Loads existing data from a CSV
        for plotting (skips execution).

        Args
        ----
        code_stabilizers : List[str] Pauli strings defining the code
        stabilizers. toggles : List[str] Pauli strings for recovery toggles.
        measure_stabilizers : List[str] Pauli strings to measure each round.
        logical_operators : Dict[str, str] Mapping from logical operator labels
        to Pauli strings. ancillas : int Number of ancilla qubits. psi_circuit
        : Any stim.Circuit preparing the initial state. p_spam : float SPAM
        error probability. param_names : str or List[str] Name(s) of noise
        parameters to sweep. param_values : List[float] or Dict[str,
        List[float]] Shared list (sync) or per-parameter lists (async) of
        values. round_list : List[int] MDR round indices to record. shots :
        int, optional Shots per Monte Carlo measurement. Default is 1000.
        num_replicates : int, optional Independent repeats per combo. Default
        is 30. split_2q : bool, default True If True, divides the input
        probability `p` evenly among all active parameters of the same type
        (1-qubit or 2-qubit). This prevents probabilities summing > 1 in Stim.
        save_data_filename : Optional[str] If provided, saves the simulation
        results to this CSV path after running. load_data_filename :
        Optional[str] If provided, SKIPS simulation and loads data from this
        CSV. Useful for plotting previously run jobs.
        """
        ...
        if isinstance(param_values, list):
            ...
            self.param_values_map = {
                k: param_values[:] for k in self.param_names
            }
        else:
            self.sync = False
    ...
    def _perform_sweep(
        ...
        Implements the Probability Splitting Logic: - 1-qubit params get the
        full value `p` (or p/N if N params active). - 2-qubit params get `p /
        num_2q_params` if split_2q is True.

        Updates: - Includes strict safety check to ensure probabilities sum <=
        1.0. - Uses a 0.999 safety factor if sum approaches 1 to counteract
        string formatting rounding up in Stim (e.g. {:.6g}).

        Returns
        -------
        all_means : Dict[tuple, Dict[int, Dict[str, float]]] combo → round →
        {label: mean ⟨|O|⟩} all_stds : Dict[tuple, Dict[int, Dict[str, float]]]
        ...
        one_q_params_in_sweep = [
            n for n in self.param_names if n in self.single_params
        ]
        two_q_params_in_sweep = [
        ...
        for combo in self.param_combos:
            ...
            for name, val in zip(self.param_names, combo):
                if name in self.single_params:
                    # FIX: Split 1Q noise if multiple components exist (e.g.
                    # Unbiased X/Y/Z)
                    # This prevents p_x=0.5, p_y=0.5, p_z=0.5 -> Sum=1.5
                    # (Error)
                    ...
                    # Case B: 2-qubit parameter. Apply split value based on
                    # number of active 2Q params.
            ...
            # We use a safety factor of 0.999 (0.1% margin) to effectively
            # "round down"
            ...
            if sum_2q > 1.0 - 1e-9:  # Check if close to or exceeding 1
                ...
                kwargs["gate_noise_2q"] = [
                    p * scale_factor for p in kwargs["gate_noise_2q"]
                ]
    ...
    def save_results(self, filename: str) -> None:
        ...
        The CSV structure will be: [param_1, param_2, ..., round, operator,
        mean, std]
    ...
    def load_results(self, filename: str) -> None:
        ...
        This allows a new MdrNoiseSweep object to behave as if it had run the
        simulation, enabling the usage of plotting methods.
        ...
        for _, row_combo in unique_combos_df.iterrows():
            ...
            # Assuming sync for plot_multi x-axis
            self.param_values_list.append(combo[0])
        ...
        self.measure_stabilizers = sorted(
            [op for op in unique_ops if op.startswith("S_")]
        )

        # Reconstruct logicals as a dict (values are placeholders, keys needed
        # for plotting)
        ...
        # For plotting, if we have >1 param name but they change together in
        # the file,
        # we treat it as sync.
        self.sync = True
        self.param_values_map = {
            p: unique_combos_df[p].tolist() for p in self.param_names
        }
    ...
    def plot_expectations_vs_param(
        ...
        Args
        ----
        category : {'stabilizer', 'logical'} Which operators to plot. rounds :
        List[int] MDR round indices to include. subset : Optional[List[str]] If
        provided, restrict to these operator labels. overlay : bool If True,
        overlay all rounds on a single axis. cols : int Number of columns for
        grid when overlay=False. figsize : Tuple[int,int] Figure size in
        inches. save_path : Optional[str] Path to save the figure (dpi=300).
        log_x : bool If True, set x-axis to logarithmic scale.

        Raises
        ------
        ValueError If category invalid or subset contains unknown labels.
    ...
    def plot_multi(
        ...
        """
        Overlay ⟨|O|⟩ vs p for multiple sweeps or grid by round, with ±1σ
        errorbars and optional straight-line fits.

        Args:
        sweeps: Dict mapping legend → MdrNoiseSweep instance. category:
        'stabilizer' or 'logical'. rounds: List of MDR rounds to plot. subset:
        If provided, restrict to these operator labels. overlay: If True,
        overlay all rounds on one axis. log_x: If True, set x-axis to
        logarithmic scale. fit: If True, overplot solid linear fits (excluding
        p=0). figsize: Figure size (width, height) in inches. save_path: If
        given, save figure (dpi=300) before showing.
        ...
        if overlay:
            style_keys = [
                (m, lbl, r) for m in sweeps for lbl in labels for r in rounds
            ]
        else:
            style_keys = [(m, lbl) for m in sweeps for lbl in labels]
        style_map = {
            key: (
            ...
            for model, sweep in sweeps.items():
                ...
                if overlay:
                    for lbl in labels:
                        for rr in rounds:
                            means = np.array(
                                [sweep.results[c][rr][lbl] for c in combos]
                            )
                            stds = np.array(
                                [sweep.results_std[c][rr][lbl] for c in combos]
                            )
    ...
    def plot_error_multi(
        ...
        """
        Plot error rate = 1 − ⟨|O|⟩ for multiple sweeps, connecting means with
        solid lines and ±1σ error bars.

        Args
        ----
        sweeps : Dict[str, MdrNoiseSweep] Map legend→sweep instance (requires
        .results & .results_std). category : {'stabilizer','logical'} Which
        operators to plot. rounds : List[int] MDR round indices to include.
        subset : Optional[List[str]] If provided, restrict to these operator
        labels. overlay : bool If True, overlay all rounds on one axis. log_x :
        bool If True, set x-axis to logarithmic scale. figsize : Tuple[int,int]
        Figure size in inches. save_path : Optional[str] Path to save the
        figure (dpi=300).

        Raises
        ------
        ValueError If sweeps empty or category/subset invalid.
            ...
            for model, sweep in sweeps.items():
                ...
                if overlay:
                    for op in labels:
                        for r in rounds:
                            means = np.array(
                                [sweep.results[c][r][op] for c in combos]
                            )
                            stds = np.array(
                                [sweep.results_std[c][r][op] for c in combos]
                            )
    ...
    def table_round_delta(
        ...
        *,
        ...
        Args
        ----
        rounds : Tuple[int, int] (r1, r2) MDR round indices to compare (both
        must be in ``self.round_list`` and must differ). category :
        {'stabilizer', 'logical'}, default 'logical' Which operator set to
        analyse. subset : list[str] | None, default None Restrict to these
        operator labels if provided. precision : int, default 4 Number of
        decimal places when printing the table. return_df : bool, default False
        If True, also return the :class:`pandas.DataFrame`.

        Returns
        -------
        pandas.DataFrame | None A tidy table whose columns are * ``param``   –
        the swept noise parameter (or tuple of parameters) * ``Δ<label>`` –
        change μ(r₂) − μ(r₁) for each operator * ``Δavg``    – arithmetic mean
        of all Δ columns. Returned only when *return_df* is True.
        """
        r1, r2 = rounds
        if r1 == r2:
