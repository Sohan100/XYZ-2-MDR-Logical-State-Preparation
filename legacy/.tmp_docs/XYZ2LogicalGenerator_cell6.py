...
class XYZ2LogicalGenerator:
    ...
    Logical X: A diagonal strip of X operators. - Logical Y: A specific
    "butterfly" pattern centered at ((d-1)/2, (d-1)/2). - Logical Z: Calculated
    as the product Logical X * Logical Y.

    Attributes
    ----------
    d : int Code distance. n : int Total number of qubits (2 * d^2).
    _stabilizer_gen : XYZ2StabilizerGenerator Internal instance for coordinate
    mapping logic.

    Methods
    -------
    generate_logicals() Return a dictionary containing 'Logical X', 'Logical
    Y', and 'Logical Z'. _get_logical_x() Construct the Logical X string.
    _get_logical_y() Construct the Logical Y string. _multiply_paulis(op_a,
    op_b) Compute the product of two Pauli strings (ignoring global phase).

    Example
    -------
    >>> log_gen = XYZ2LogicalGenerator(distance=3) >>>
    log_gen.generate_logicals() {'Logical X': 'X4 X5', 'Logical Y': 'X4 Z7 Y10
    X13', 'Logical Z': ...}
    """
    ...
    def generate_logicals(self) -> Dict[str, str]:
        ...
        Returns
        -------
        Dict[str, str] Dictionary keys: "Logical X", "Logical Y", "Logical Z".
        ...
        return {"Logical X": lx, "Logical Y": ly, "Logical Z": lz}
    ...
    def _get_logical_x(self) -> str:
        ...
        Construct Logical X along the main diagonal. Corresponds to 'lower'
        vertices where j = d - 1 - i.
    ...
    def _get_logical_y(self) -> str:
        ...
        Structure: 1. Center block (c,c): Z on upper vertex, Y on lower vertex.
        2. Lower diagonal (i+j = d-2): X on lower vertices. 3. Upper diagonal
        (i+j = d): X on upper vertices.
        """
        c = (self.d - 1) // 2
    ...
    def _multiply_paulis(self, a_str: str, b_str: str) -> str:
        ...
        Used to derive Logical Z = Logical X * Logical Y. Ignores global phase.

        Args:
        a_str: First Pauli string (e.g., "X0 Z1"). b_str: Second Pauli string
        (e.g., "Y0 X2").
