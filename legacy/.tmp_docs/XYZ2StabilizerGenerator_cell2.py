...
XYZ2StabilizerGenerator_cell2.py
----------------------------------------------------------------------------
...
class XYZ2StabilizerGenerator:
    ...
    The XYZ^2 code is defined on a honeycomb lattice with lattice parameter
    `d`. The code typically employs: - Weight-2 XX links on vertical edges. -
    Weight-6 XYZXYZ plaquettes in the bulk. - Weight-3 boundary checks.

    Attributes
    ----------
    d : int Code distance (must be odd and >= 3). n : int Total number of
    qubits (2 * d^2). _layer_offsets : list[int] Precomputed offsets for
    mapping (i,j) coordinates to qubit indices.

    Methods
    -------
    generate_stabilizers() Return a list of sparse Pauli strings representing
    the stabilizers. _coord_to_verts(i, j) Map 2D lattice coordinates (i, j) to
    the pair of qubit indices (up, lo).

    Example
    -------
    >>> gen = XYZ2StabilizerGenerator(distance=3) >>>
    gen.generate_stabilizers() ['X0 X1', 'Y0 Z1 X2', ...]
    """
    ...
    def __init__(self, distance: int) -> None:
        ...
        # diag_counts represents the number of vertical links in each diagonal
        # slice
        diag_counts = [
    ...
    def generate_stabilizers(self) -> List[str]:
        ...
        Returns
        -------
        List[str] List of Pauli strings (e.g., ["X0 X1", "Y0 Z1 X2", ...]).
        """
        stabs: List[str] = []
    ...
    def _coord_to_verts(self, i: int, j: int) -> Tuple[int, int]:
        ...
        Args:
        i: Row index (0 to d-1) j: Column index (0 to d-1)
