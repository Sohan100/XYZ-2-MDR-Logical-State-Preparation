#!/bin/bash
#SBATCH --job-name=surface_parallel
#SBATCH --output=surface_parallel_%j.out
#SBATCH --error=surface_parallel_%j.err
#SBATCH -C cpu
#SBATCH -q regular
#SBATCH -t 47:30:00
#SBATCH --nodes=18
...
# Optional manual override. Leave empty to auto-resolve the latest family run.
RUN_NAME_OVERRIDE=""
NUM_SHOTS=3000
...
WORKING_FOLDER_FILE="${FAMILY_ROOT}/working-folder-${CODE_FAMILY}-${EXPECTED_CODE_NAME}.txt"
FAMILY_WORKING_FOLDER_FILE="${FAMILY_ROOT}/working-folder-${CODE_FAMILY}.txt"
LEGACY_FAMILY_WORKING_FOLDER_FILE="${FAMILY_ROOT}/working-folder.txt"
LEGACY_ROOT_CODE_FILE="${ROOT_DIR}/working-folder-${EXPECTED_CODE_NAME}.txt"
LEGACY_ROOT_WORKING_FOLDER_FILE="${ROOT_DIR}/working-folder.txt"

if [[ -n "${RUN_NAME_OVERRIDE}" ]]; then
    RUN_NAME="${RUN_NAME_OVERRIDE}"
elif [[ -f "${WORKING_FOLDER_FILE}" ]]; then
    RUN_NAME="$(tr -d '[:space:]' < "${WORKING_FOLDER_FILE}")"
elif [[ -f "${FAMILY_WORKING_FOLDER_FILE}" ]]; then
    RUN_NAME="$(tr -d '[:space:]' < "${FAMILY_WORKING_FOLDER_FILE}")"
elif [[ -f "${LEGACY_FAMILY_WORKING_FOLDER_FILE}" ]]; then
    RUN_NAME="$(tr -d '[:space:]' < "${LEGACY_FAMILY_WORKING_FOLDER_FILE}")"
elif [[ -f "${LEGACY_ROOT_CODE_FILE}" ]]; then
    RUN_NAME="$(tr -d '[:space:]' < "${LEGACY_ROOT_CODE_FILE}")"
elif [[ -f "${LEGACY_ROOT_WORKING_FOLDER_FILE}" ]]; then
    RUN_NAME="$(tr -d '[:space:]' < "${LEGACY_ROOT_WORKING_FOLDER_FILE}")"
else
    shopt -s nullglob
    run_candidates=( "${FAMILY_ROOT}"/Run-* )
    shopt -u nullglob
    if (( ${#run_candidates[@]} > 0 )); then
        latest_run_path=""
        for candidate in "${run_candidates[@]}"; do
            if [[ -z "${latest_run_path}" || "${candidate}" -nt "${latest_run_path}" ]]; then
                latest_run_path="${candidate}"
            fi
        done
...
if [[ -z "${RUN_NAME}" ]]; then
    echo "Could not determine RUN_NAME from family working-folder files or Run-*"
    exit 1
fi
