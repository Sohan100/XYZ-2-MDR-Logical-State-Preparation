...
#SBATCH --output=xyz2_link_final_%A_%a.out
#SBATCH --error=xyz2_link_final_%A_%a.err
#SBATCH -C cpu
#SBATCH -q regular
#SBATCH -t 47:30:00
#SBATCH --nodes=1
...
# Each task runs all requested rounds for its probability point. The final
# completed task for a distance/noise merge creates the merged CSV, and the
# final completed merge creates the Logical-X and all-operator plots once.
...
RUN_SUFFIX="${PREP_MODE}-${RECOVERY_MODE}"
